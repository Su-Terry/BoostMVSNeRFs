...
class Dataset:
    ...
    def build_metas(self):
        ...
        for scene in scenes:
            ...
            all_ids = [i for i in range(len(img_paths)) if i % 2 == 0]
            train_ids = [all_ids[i] for i in range(len(all_ids)) if i % 8 != 0]
            if self.split == 'train':
                render_ids = train_ids
            else:
                render_ids = [all_ids[i] for i in range(len(all_ids)) if i % 8 == 0]
