...
class Dataset:
    def __init__(self, **kwargs):
        ...
        im = np.array(im)
        h, w = self.input_h_w_ori[:2]
        im = im[32:-32, 32:-32]
        self.input_h_w = im.shape
        # self.final_h_w = [384, 576]
    ...
    def build_metas(self):
        ...
        if os.path.exists('/home/public/ENeRF_workspace/result/mvs_ours/dtu_pretrain/scannet/mcp_outputs.json'):
            import json
            with open(f'/home/public/ENeRF_workspace/result/mvs_ours/dtu_pretrain/scannet/mcp_outputs.json', 'r') as f:
                mcp_outputs = json.load(f)
        else:
            mcp_outputs = None
        ...
        for scene in scenes:
            ...
            if mcp_outputs:
                train_ids = [i for i in train_ids if mcp_outputs[f'{scene}_{i}'] != []]
    ...
    def read_image(self, scene, view_idx):
        ...
        img = np.array(img)
        h, w = img.shape[:2]
        # img = img[int(h*0.1):int(h*0.9), int(w*0.05):int(w*0.95)]
        img = img[32:-32, 32:-32] # crop 32*2
