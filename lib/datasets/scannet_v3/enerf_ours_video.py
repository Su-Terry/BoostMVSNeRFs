...
class Dataset:
    ...
    def build_metas(self):
        ...
        for scene in scenes:
            ...
            """"""
            poses_render = np.load(os.path.join(self.data_root, scene, 'poses_render.npy')) # c2w, -u, r, -t
            # poses_render = pose_renders[:, :15].reshape((-1, 3, 5))
            c2ws_render = np.eye(4)[None].repeat(len(poses_render), 0)
            c2ws_render[:, :3, :4] = poses_render[:, :3, :4]
            c2ws_render[:, :3, 1] *= -1
            c2ws_render[:, :3, 2] *= -1
            # c2ws_render[:, :3, 0], c2ws_render[:, :3, 1], c2ws_render[:, :3, 2], c2ws_render[:, :3, 3] = poses_render[:, :3, 1], poses_render[:, :3, 0], -poses_render[:, :3, 2], poses_render[:, :3, 3]
            ixts_render = np.tile(ixt, (len(poses_render), 1, 1))
            # ixts_render[:, 0, 2], ixts_render[:, 1, 2] = poses_render[:, 1, 4]/2., poses_render[:, 0, 4]/2.
            # ixts_render[:, :2] *= 0.5
            # depth_ranges_render = pose_renders[:, -2:]
            """"""
            ...
            scene_info = {'ixts': ixts.astype(np.float32), 
                          'c2ws': c2ws.astype(np.float32),
                          'image_names': image_names, 
                          'depth_ranges': depth_ranges.astype(np.float32),
                          
                          'c2ws_render': c2ws_render.astype(np.float32),
                          'ixts_render': ixts_render.astype(np.float32),
                        #   'depth_ranges_render': depth_ranges_render.astype(np.float32)
                        }
            ...
            if os.path.exists(train_file):
                ...
                render_ids = list(range(len(c2ws_render)))
                if self.split == 'train':
    ...
    def read_tar(self, scene, view_idx):
        ...
        if self.split == 'train':
            ...
            ixt, ext, _ = self.read_cam(scene, view_idx, orig_size, is_render=True)
        mask = np.ones_like(img[..., 0]).astype(np.uint8)
        return img, mask, ext, ixt

    def read_cam(self, scene, view_idx, orig_size, is_render=False):
        if is_render:
            ixt = scene['ixts_render'][view_idx].copy()
            ext = scene['c2ws_render'][view_idx].astype(np.float32)
        else:
