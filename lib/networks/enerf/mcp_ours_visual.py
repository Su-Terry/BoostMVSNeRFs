...
class MCP(nn.Module):
    ...
    def calc_mask(self, src_views_id, cam_tar, ray_tar, batch):
        ...
        for i in range(cfg.enerf.cas_config.num):
            ...
            file_name = f'./masks/{batch["meta"]["scene"][0]}_{batch["meta"]["tar_view"].detach().cpu().numpy()[0]}_{src_views_id.detach().cpu().numpy().tolist()}_{i}.png'
            imageio.imwrite(file_name, mask_rgb)
