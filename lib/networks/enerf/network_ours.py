...
class Network(nn.Module):
    ...
    def merge_mlp_outputs(self, outputs, batch, N_CV):
        ...
        # outputs = []
        # for i in range(N_CV):
        #     outputs.append(utils.raw2outputs_blend(net_outputs[:, i:i+1], torch.ones_like(masks[:, i:i+1]), z_vals[:, i:i+1], cfg.enerf.white_bkgd)['rgb'])
        
        masks_sum = masks.sum(1)
        masks = torch.where(masks_sum > 0, masks / masks_sum, 1 / N_CV)
        # net_outputs[..., -1] = net_outputs[..., -1] * masks
        # density_sum = net_outputs[..., -1].sum(1)
        # net_outputs[..., -1] /= density_sum
    ...
    def forward(self, batch):        
        ...
        for i in range(cfg.enerf.cas_config.num):
            ...
            # volume_render_ret[f'rgb_level{i}'] = volume_render_ret[f'rgb_level{i}'].reshape(-1, H, W, 3).permute(0, 3, 1, 2)
            # volume_render_ret[f'rgb_level{i}'] = F.interpolate(volume_render_ret[f'rgb_level{i}'], size=(h, w), mode='bilinear')
            # B = 1
            # volume_render_ret[f'rgb_level{i}'] = volume_render_ret[f'rgb_level{i}'].permute(0, 2, 3, 1).reshape(B, -1, 3)
