...
class Network(nn.Module):
    def __init__(self,
        ...
        self.embed_fn, _ = get_embedder(10, 0, input_dims=3)
    ...
    def build_volume_costvar(self, feats, proj_mats, depth_values, pad=0):
        ...
        del volume_sq_sum, volume_sum, count, in_masks
    ...
    def build_volume_costvar_img(self, imgs, feats, proj_mats, depth_values, pad=0):
        ...
        for i, (src_img, src_feat, proj_mat) in enumerate(zip(imgs[1:], src_feats, proj_mats)):
            ...
            in_mask = ((grid > -1.0) * (grid < 1.0))
            in_mask = (in_mask[..., 0] * in_mask[..., 1])
            in_masks[:, i+1:i+2] = in_mask.float()
...
#####################################################################################################################
    def ray_marcher(self, rays, N_sample, lindisp=False, perturb=0, bbox_3D=None):
        ...
        zsteps = torch.linspace(0., 1., steps=N_sample, device=rays.device) # .unsqueeze(0).expand(B, -1)  # (B, N_sample)
        if not lindisp:
            z_vals = near * (1. - zsteps) + far * zsteps
        else:
            z_vals = 1. / (1. / near * (1. - zsteps) + 1. / far * zsteps)

        xyz_coarse_sampled = rays_o.unsqueeze(2) + rays_d.unsqueeze(2) * z_vals.unsqueeze(3)  # (B, N_rays, N_sample, 3)
    ...
    def render_rays(self, rays, **kwargs):
        ...
        for i in range(0, N_rays, chunk):
            ...
            H, W = int(H_O * cfg.enerf.cas_config.render_scale[level]), int(W_O * cfg.enerf.cas_config.render_scale[level])
...
#####################################################################################################################
    ...
    def get_proj_mats(self, batch):
        B = batch['all_src_inps'].shape[0]
        proj_mats_batch = []
        
        for b in range(B):
            proj_mats = []
            for i, (src_ext, src_ixt) in enumerate(zip(batch['src_exts'][b], batch['src_ixts'][b])):
                proj_mat_l = torch.eye(4, device=src_ext.device)
                ...
                if i==0:
                    ...
                    proj_mats.append(torch.eye(4, device=src_ext.device))
                else:
                    proj_mats.append(proj_mat_l @ ref_proj_inv)
            
            proj_mats_batch.append(torch.stack(proj_mats)[:, :3])

        return torch.stack(proj_mats_batch).float()
    ...
    def forward(self, batch):
        ...
        mlp_level_ret = {}
        t_vals = torch.linspace(0., 1., steps=D, device=device, dtype=batch['all_src_inps'].dtype) .unsqueeze(0).expand(B, -1) # (B, D)
        v = 0
        views = torch.tensor([0, 1, 2]).to(device)
        near, far = (batch['depth_ranges'][:, views].min()*0.8).to(device), (batch['depth_ranges'][:, views].max()*1.2).to(device)
        depth_values = near * (1.-t_vals) + far * (t_vals)
        depth_values = depth_values.to(device)

        batch['near_far'] = torch.stack([near, far]).to(device)
