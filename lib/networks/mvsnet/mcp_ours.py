...
#####################################################################################################################
    ...
    def render_rays(self, rays, **kwargs):
        ...
        world_xyz, z_vals = self.ray_marcher(rays[0], 128)
...
#####################################################################################################################
    ...
    def calc_mask(self, src_views_id, cam_tar, ray_tar, batch):
        ...
        # feats = self.feature(batch['src_inps'][0])
        ...
        # for i in range(cfg.enerf.cas_config.num):
            ...
            # volume_feat, _ = self.build_volume_costvar_img(batch['src_inps'][0], feats, proj_mats, batch['depth_values'], pad=24)
    ...
    def forward(self, batch):
        ...
        for i, src_views_id in enumerate(selected_views):
            src_masks = self.calc_mask(src_views_id, batch['tar_ext'], batch['rays_0'][0], batch)
