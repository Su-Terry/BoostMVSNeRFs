...
class Visualizer:
    ...
    def summarize(self):
        ...
        d_min, d_max = np.min(self.depths), np.max(self.depths)
        # self.depths = [ (dpt - d_min)/(d_max - d_min) for dpt in self.depths ]
        self.depths = [ (dpt - dpt.min())/(dpt.max() - dpt.min()) for dpt in self.depths ]
        self.depths = [(depth * 255).astype(np.uint8) for depth in self.depths]
        self.depths = [cv2.applyColorMap(depth, cv2.COLORMAP_JET) for depth in self.depths]
